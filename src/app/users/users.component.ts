    ...
    if (!this.firstNameError && !this.lastNameError && !this.employeeIDError) {
      ...
      this.apiService.AddUser(this.object)
        .subscribe((data: any) => {
          this.ResetData();
          this.GetUsers();
          document.getElementById('userMsg').innerText = "New User Successfully Added to System";
          document.getElementById('userMsg').style.color = "green";
        },
          function (error) {
            console.log(error);
            document.getElementById('userMsg').innerText = "Application Error, Please check with support team";
    ...
    if (!this.firstNameError && !this.lastNameError && !this.employeeIDError) {
      ...
      this.apiService.UpdateUser(this.object)
        .subscribe((data: any) => {
          this.ResetData();
          this.GetUsers();
          document.getElementById('userMsg').innerText = "User Information Successfully Updated";
          document.getElementById('userMsg').style.color = "green";
        },
          function (error) {
            console.log(error);
  ...
  DeleteUser(user) {
    ...
    this.apiService.DeleteUser(obj)
      .subscribe((data: any) => {
        this.ResetData();
        this.GetUsers();
        document.getElementById('userMsg').innerText = "User is removed from the System Successfully";
        document.getElementById('userMsg').style.color = "green";
      },
        function (error) {
          console.log(error);
